...
if(game&&copy){
 ...
 const related=getRelatedGames(game.id,3);document.getElementById('relatedTitle').textContent=copy.related;document.getElementById('relatedGames').innerHTML=related.map(g=>`<article class="game-card"><a href="${getGameUrl(g.id)}"><div class="game-image-container"><img class="game-image" src="${assetPath(g.image)}" alt="Preview of ${g.name[pageLang]||g.name.en}" loading="lazy"></div></a><div class="game-info"><h3 class="game-name">${g.name[pageLang]||g.name.en}</h3><p class="game-genre">${g.genre}</p><a class="btn game-play-btn" href="${getGameUrl(g.id)}">PLAY</a></div></article>`).join('');
}
